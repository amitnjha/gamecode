...
bool InputSystem::Initialize()
{
	...
	cout << "SDL_NumJoysticks() : " << SDL_NumJoysticks() << "\n";
	for (int i = 0; i < SDL_NumJoysticks(); i++) {
	  if (SDL_IsGameController(i)) {
	    // Found a game controller, open it with SDL_GameControllerOpen(i);
	    cout << "Found a controller" << "\n";
	  }else{
	    cout << "Found a non controller" << "\n";
	  }
	}
	if (mController != nullptr)
	  cout << "Controller is found " << SDL_IsGameController(0) << "\n";
	else
	  cout << "Controller is not found " << SDL_IsGameController(0)  << "\n";
